...
I ran into a few issues when coding the UI. I was stuck on the following error for quite a while (Error in match.arg(position) : 'arg' must be NULL or a character vector), before noticing that the selectInput() function I was trying to add was outside the closing parenthesis of the sidebar panel. Once I moved it within the final parenthesis, the issue resolved. 

Figure 8: The three inputs users can adjust. The first is a slider to select the year of the data to display. The second is a dropdown selection to choose which funding variable to map. The third allows users to change the color palette of the mapped data via radio buttons.
...
The figure below provides an example of the reactivity of the map. In the gif, the fill variable (funding program) is selected at PK -12 Education. The different frames of the gif show the changes made to the map when the year selection changes. The final code for the app can be seen (https://github.com/tessaeagle/governmentspending)[here]. <br>
